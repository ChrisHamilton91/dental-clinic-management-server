...
app.post("/assign-employee-to-dentist", async (req, res) => {
  try {
    ...
    const result = await db.func(
      ...
      [c.first_name, c.last_name, c.ssn, c.email],
      queryResult.one
    );
...
app.post("/create-branch", async (req, res) => {
  try {
    ...
    const result = await db.func(
      "create_branch",
      [b.first_name, b.last_name, b.ssn, b.email, b.city],
      queryResult.one
    );
...
app.patch("/update-patient-with-patient-id", async (req, res) => {
  try {
    ...
    await db.proc("update_patient_with_patient_id", [
      patient_id,
      house_number,
      street,
      city,
...
app.patch("/update-patient-with-person-id", async (req, res) => {
  try {
    ...
    await db.proc("update_patient_with_person_id", [
      person_id,
      house_number,
      street,
      city,
...
//Get procedure types
app.get("/get-procedure-types", async (_, res) => {
  try {
...
//Get all branches
app.get("/get-all-branches", async (_, res) => {
  try {
    const reply = await db.func("get_all_branches");
    res.send(reply);
  } catch (err) {
    console.error(err.message);
    res.status(500).send(err);
